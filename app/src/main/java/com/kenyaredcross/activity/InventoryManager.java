...
public class InventoryManager extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void checkUserProfile() {
        ...
        databaseReference.child("Profiles").child(userId).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onCancelled(@NonNull DatabaseError error) {
                Toast.makeText(InventoryManager.this, "Error checking profile: " + error.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void showProfilePopup() {
        ...
        databaseReference.child("Users").child(auth.getCurrentUser().getEmail().replace(".", "_")).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                Toast.makeText(InventoryManager.this, "Error retrieving username: " + error.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
        ...
        DatePickerDialog datePickerDialog = new DatePickerDialog(
                ...
                (view, year1, month1, dayOfMonth) -> {
                    ...
                    if (selectedDate.after(Calendar.getInstance())) {
                        Toast.makeText(InventoryManager.this, "Date of Birth cannot be in the future", Toast.LENGTH_SHORT).show();
                    } else {
        ...
        databaseReference.child("Profiles").child(userId).setValue(profile)
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
                        Toast.makeText(InventoryManager.this, "Profile Updated!", Toast.LENGTH_SHORT).show();
                        dialog.dismiss();
                    } else {
                        Toast.makeText(InventoryManager.this, "Failed to save profile", Toast.LENGTH_SHORT).show();
                    }
                });
    }
